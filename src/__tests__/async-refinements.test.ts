...
test("parse async test", async () => {
  const schema1 = z.sString().refine(async (_val) => false);
  expect(() => schema1.parse("asdf")).toThrow();

  const schema2 = z.sString().refine((_val) => Promise.resolve(true));
  return await expect(() => schema2.parse("asdf")).toThrow();
});

test("parseAsync async test", async () => {
  const schema1 = z.sString().refine(async (_val) => true);
  await schema1.parseAsync("asdf");

  const schema2 = z.sString().refine(async (_val) => false);
...
test("parseAsync async test", async () => {
  ...
  const schema1 = z.sString().refine((_val) => Promise.resolve(true));
  const v1 = await schema1.parseAsync("asdf");
  expect(v1).toEqual("asdf");

  const schema2 = z.sString().refine((_val) => Promise.resolve(false));
  await expect(schema2.parseAsync("asdf")).rejects.toBeDefined();

  const schema3 = z.sString().refine((_val) => Promise.resolve(true));
...
test("parseAsync async with value", async () => {
  const schema1 = z.sString().refine(async (val) => {
    return val.length > 5;
  });
